...
def populate_options_from_config(config):
    global opt
    store_true = ["skip_grid", "skip_save", "plms", "laion400m", "fixed_code"]
...
def module_init():
    ...
    parser.add_argument(
        ...
        default="stable-diffusion/outputs/txt2img-samples"
    )
    parser.add_argument(
        "--skip_grid",
        action='store_true',
    ...
    global opt, device, model, config, outpath, wm_encoder
    opt = parser.parse_args()

    # Choice needs to be made at app start-up. Not allowing to choose this on a per run basis
    if opt.laion400m:
...
def paint(task_id, job_config):
    ...
    print("Painter config: prompt: %s, isPlms: %s, n_iter: %s" % (opt.prompt, str(opt.plms), opt.n_iter))
